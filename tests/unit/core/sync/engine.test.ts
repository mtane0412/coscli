...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "coscli-engine-test-"))
})
...
describe("syncPull", () => {
  test("ページをローカルに取得してメタファイルを生成する", async () => {
    ...
    await syncPull(client, testDir, "テストプロジェクト", "テストページ")

    // ローカルファイルが作成されている
    expect(existsSync(join(testDir, "テストページ.txt"))).toBe(true)

    // メタファイルが作成されている
    const meta = readMeta(testDir, "テストプロジェクト", "テストページ")
  ...
  test("タイトル行 (lines[0]) を除いた本文が保存される", async () => {
    ...
    const result = await syncPull(client, testDir, "テストプロジェクト", "テストページ")
  ...
  test("--dry-run 時はファイルを書き込まない", async () => {
    ...
    await syncPull(client, testDir, "テストプロジェクト", "テストページ", { dryRun: true })

    expect(existsSync(join(testDir, "テストページ.txt"))).toBe(false)
  })
...
describe("syncPush", () => {
  test("ローカルファイルをリモートに push する", async () => {
    ...
    // ローカルファイルを編集
    writeFileSync(join(testDir, "テストページ.txt"), "編集した本文\n", "utf-8")

    const writer = makeWriter("新コミットXYZ")
    const result = await syncPush(client, writer, testDir, "テストプロジェクト", "テストページ")
  ...
  test("メタが無いとき META_REQUIRED エラーを返す", async () => {
    writeFileSync(join(testDir, "テストページ.txt"), "本文\n", "utf-8")
...
describe("syncDiff", () => {
  test("in-sync のとき in-sync ステータスを返す", async () => {
    ...
    const result = await syncDiff(client, testDir, "テストプロジェクト", "テストページ")
  ...
  test("リモートが空でローカルにある場合 local-only ステータス", async () => {
    ...
    writeFileSync(join(testDir, "ローカルのみ.txt"), "ローカルの行\n", "utf-8")
    ...
    const result = await syncDiff(client, testDir, "テストプロジェクト", "ローカルのみ")
  ...
  test("ローカルの変更を差分として返す", async () => {
    ...
    writeFileSync(join(testDir, "テストページ.txt"), "新しい本文\n追加行\n", "utf-8")
...
describe("commitId が欠落した実 API レスポンスへのフォールバック", () => {
  ...
  test("syncPush: サーバの commitId が欠落かつローカルも空文字列のとき競合なしで push できる", async () => {
    ...
    await syncPull(pullClient, testDir, "テストプロジェクト", "テストページ")

    // ローカルを編集
    writeFileSync(join(testDir, "テストページ.txt"), "変更した本文\n", "utf-8")

    // サーバも commitId なしのまま (他者更新なし)
    const pushClient = makeRestClient(page)
    const writer = makeWriter("新コミット123")
    const result = await syncPush(pushClient, writer, testDir, "テストプロジェクト", "テストページ")
  ...
  test("syncDiff: commitId なしページを pull した後の差分検出が正常に動作する", async () => {
    ...
    // ローカルを変更
    writeFileSync(join(testDir, "テストページ.txt"), "変更後の本文\n", "utf-8")
