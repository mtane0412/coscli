...
describe("buildRestClient — COS_SID 環境変数に PAT が設定された場合 (Phase 6: hard error)", () => {
  let exitMock: ReturnType<typeof spyOn>
  let stdoutMock: ReturnType<typeof spyOn>
  ...
  it("COS_SID に有効な PAT が設定されている場合は exit 5 + INVALID_SID で終了する (互換モード廃止)", async () => {
    // Phase 6: COS_SID に PAT を設定することは禁止。互換モードを廃止して hard error に変更。
    // PAT は COS_PERSONAL_ACCESS_TOKEN 環境変数で指定する必要がある。
  ...
  it("COS_SID に不正な PAT (pat_ プレフィックスだが形式が不正) が設定されている場合も exit 5 + INVALID_SID で終了する", async () => {
    // Phase 6: pat_ プレフィックスがあれば即座に INVALID_SID で拒否する
...
describe("requireSid — PAT 誤投入の明示拒否 (Phase 6: COS_SID への PAT は hard error)", () => {
  let exitMock: ReturnType<typeof spyOn>
  let stdoutMock: ReturnType<typeof spyOn>
  ...
  it("COS_SID 環境変数に PAT を設定した場合は exit 5 + INVALID_SID で終了する (Phase 6 hard error)", async () => {
    // Phase 6: COS_SID に PAT を設定することは禁止 (互換モード廃止)。
    // resolveActiveCredential が先に INVALID_SID で exit 5 するため、requireSid まで到達しない。
    // PAT は COS_PERSONAL_ACCESS_TOKEN 環境変数で指定する必要がある。
    process.env["COS_SID"] = VALID_PAT
    try {
      await requireSid()
  ...
  it("エラー JSON に hint (COS_PERSONAL_ACCESS_TOKEN 環境変数への案内) が含まれること", async () => {
    // Phase 6: COS_SID への PAT 設定は INVALID_SID で拒否し、COS_PERSONAL_ACCESS_TOKEN を使うよう案内する
    process.env["COS_SID"] = VALID_PAT
    try {
      await requireSid()
    } catch {
    ...
    const stdoutOutput = (stdoutMock.mock.calls as Array<[string]>).map((c) => c[0]).join("")
    // ヒントに COS_PERSONAL_ACCESS_TOKEN への誘導が含まれること
    expect(stdoutOutput).toContain("COS_PERSONAL_ACCESS_TOKEN")
  })
})
